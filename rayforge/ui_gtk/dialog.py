...
def show_platform_dialog(
    ...
    if sys.platform == "darwin":
        ...
        sorted_buttons = sorted(
            buttons, 
            key=lambda b: (not b.get('is_default', False), not b.get('is_cancel', False))
        )
        ...
        if 0 <= clicked_index < len(sorted_buttons):
            ...
            cancel_id = next((b['id'] for b in buttons if b.get('is_cancel')), buttons[-1]['id'])
            callback(cancel_id)
        return
    ...
    for btn_cfg in buttons:
        ...
        if btn_cfg.get('is_destructive'):
            dialog.set_response_appearance(b_id, Adw.ResponseAppearance.DESTRUCTIVE)
        elif btn_cfg.get('is_default'):
            dialog.set_response_appearance(b_id, Adw.ResponseAppearance.SUGGESTED)
